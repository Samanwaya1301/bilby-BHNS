...
def phase_TH_old(
    ...
    """ Phase correction due to tidal heating
        The spin-orbit interaction terms are missing plus issues with
...
#phase contribution from the primary mass; which is known to be a BH
...
def phase_TH_BH(
    ...
    #-----------------------------------------------------------------------------------------------
    #Heff5 and Heff8 defined in terms of mass and spin (new parametrization)
    H_eff5 =  (mass_1/m)**3 * LdotS1 * (3 * a_1**2 + 1) * a_1   #mass_1 is known to be a BH, 
    H_eff8 = 4 * np.pi * H_eff5 + (mass_1/m)**4 * (1 + np.sqrt(1 - a_1**2)) * (1 + 3 * a_1**2)
    #------------------------------------------------------------------------------------------------
    #2PN quadrupole term
...
#phase contribution from binary black hole with no parameters; H1 = H2 = 1 is taken
#---------------------------------------------------------------------------------------

def phase_TH_BBH_no_param(
        frequency_array, mass_1, mass_2, a_1, a_2, spin_1x, spin_1y, spin_1z,
        spin_2x, spin_2y, spin_2z, Q_tilde, start_frequency, delta_frequency):
    """ Phase correction due to tidal heating
        Added spin-orbit interaction term and corrected positive-negative sign
    ...
    #-----------------------------------------------------------------------------------------------
    #Heff5 and Heff8 defined in terms of mass and spin 
    H_eff5 =  (mass_1/m)**3 * LdotS1 * (3 * a_1**2 + 1) * a_1 + (mass_2/m)**3 * LdotS2 * (3 * a_2**2 + 1) * a_2   
    H_eff8 = 4 * np.pi * H_eff5 + (mass_1/m)**4 * (1 + np.sqrt(1 - a_1**2)) * (1 + 3 * a_1**2) + (mass_2/m)**4 * (1 + np.sqrt(1 - a_2**2)) * (1 + 3 * a_2**2)
    #------------------------------------------------------------------------------------------------
...
#phase contribution from the secondary object; Heff5 is the only extra parameter, Heff8 is defined in terms of Heff5
...
def phase_TH_secondary(
    ...
    #H_eff5 =  H2 * (mass_2/m)**3 * LdotS2 * (3 * a_2**2 + 1)  + H2 * (mass_2/m)**3 * (LdotS2) * (3 * a_2**2 + 1) #mass_1 is known to be a BH, parameter only for mass_2
    H_eff8 = 4 * np.pi * H_eff5 * a_2 * LdotS2 + H_eff5 * (mass_2/m) * (1 + np.sqrt(1 - a_2**2)) 
    
    #------------------------------------------------------------------------------------------------
    
    phase_term1 = 3.0 / (128.0 * eta * v**5)
    # 3.5 PN term ** -ve sign added
    term_v7 = - 5 * v**7 * (952 * eta + 995) / 168.0 * H_eff5 * a_2 * LdotS2
    # 2.5 PN term ** -ve sign added ***
    term_v5 = - 10 * v**5 * (3 * np.log(v) + 1) / 9. * H_eff5 * a_2 * LdotS2
    # spin-orbit interaction term
    Psi_SO = 1. / 6. * ((- 56 * eta - 73 * (np.sqrt(1 - 4 * eta) - 1)) * LdotS1 * a_1 +
                        (- 56 * eta - 73 * (np.sqrt(1 - 4 * eta) - 1)) * LdotS2 * a_2)
    # 4PN term
    term_v8 = 5 * v**8 * (3 * np.log(v) - 1) / 9.0 * (H_eff5 * a_2 * LdotS2 * Psi_SO - 4 * H_eff8)
...
def lal_binary_black_hole_tidal_heating(
        frequency_array, mass_1, mass_2, luminosity_distance, a_1, tilt_1,
        phi_12, a_2, tilt_2, phi_jl, theta_jn, phase, H_eff5, Q_tilde, **kwargs):
...
def lal_black_hole_neutron_star_tidal_heating(
    ...
    The frequencies at which we want to calculate the strain
    mass_1: float
    The mass of the heavier object(black hole) in solar masses
    mass_2: float
    The mass of the lighter object(neutron star) in solar masses
    luminosity_distance: float
    The luminosity distance in megaparsec
    a_1: float
    Dimensionless primary spin magnitude
    ...
    """
    waveform_kwargs = dict(
        waveform_approximant='HeatedTidalTaylorF2', reference_frequency=50.0,
        minimum_frequency=20.0, maximum_frequency=frequency_array[-1],
        catch_waveform_errors=False, pn_spin_order=-1, pn_tidal_order=-1,
        pn_phase_order=-1, pn_amplitude_order=0)
    waveform_kwargs.update(kwargs)
    approx = waveform_kwargs['waveform_approximant']
    if approx == 'TaylorF2' or approx == 'HeatedTaylorF2' or approx == 'HeatedTidalTaylorF2':
        f_isco = ISCO(mass_1, mass_2)
        waveform_kwargs.update(maximum_frequency=f_isco)
    return _base_lal_cbc_fd_waveform(
        frequency_array=frequency_array, mass_1=mass_1, mass_2=mass_2,
...
def _base_lal_cbc_fd_waveform(
    ...
    dict: A dictionary with the plus and cross polarisation strain modes
    """
    if waveform_kwargs['waveform_approximant'] == "HeatedTaylorF2" or waveform_kwargs['waveform_approximant'] == "HeatedTidalTaylorF2":
    ...
    try:
        if waveform_kwargs['waveform_approximant'] == "HeatedTaylorF2":
            ...
            heated_phase_primary = phase_TH_BH(
                frequency_array, mass_1, mass_2, a_1, a_2, spin_1x, spin_1y, spin_1z,
                spin_2x, spin_2y, spin_2z, Q_tilde, start_frequency, delta_frequency)
